...
PricePeg.prototype = {
  ...
  setPricePeg: function (newValue) {
    ...
    if (updatesInThisPeriod <= config.maxUpdatesPerPeriod || debugUpdates) {
      if(!disableLiveCalls) {
        client.aliasUpdate(config.pegalias, JSON.stringify(newValue), function (err, result, resHeaders) {
          if (err) {
            console.log(err);
            this.logPegMessage("Error:" + err);
            deferred.reject(err);
          } else {
            this.logUpdate(newValue);
            deferred.resolve(result);
          }
        }.bind(this));
      }else{
  ...
  logUpdate: function(update) {
    ...
    this.sysRates = update;
    this.logPegMessage("Price peg updated successfully.");
  },

  getFiatRate: function (usdRate, conversionRate) {
    var rate = 0;
  ...
  convertToPricePeg: function () {
    return {
      rates: [
        ...
        {
          "currency": "EUR",
          "rate": this.getFiatRate(this.getSYSFiatValue(CurrencyConversionType.FIAT.USD), this.fiatDataSource.formattedCurrencyConversionData.EUR),
          "precision": 2
        },
        {
          "currency": CurrencyConversionType.FIAT.GBP,
          "rate": this.getFiatRate(this.getSYSFiatValue(CurrencyConversionType.FIAT.USD), this.fiatDataSource.formattedCurrencyConversionData.GBP),
          "precision": 2
        },
        {
          "currency": CurrencyConversionType.FIAT.CAD,
          "rate": this.getFiatRate(this.getSYSFiatValue(CurrencyConversionType.FIAT.USD), this.fiatDataSource.formattedCurrencyConversionData.CAD),
          "precision": 2
        },
        {
          "currency": CurrencyConversionType.FIAT.CNY,
          "rate": this.getFiatRate(this.getSYSFiatValue(CurrencyConversionType.FIAT.USD), this.fiatDataSource.formattedCurrencyConversionData.CNY),
          "precision": 2
        },
        {
          "currency": CurrencyConversionType.CRYPTO.BTC,
          "rate": parseFloat(1 / parseFloat(this.sysBTCConversionValue)),
          "precision": 2
        },
        {
