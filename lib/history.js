...
function getHistoryPage(req, res, peg) {
  ...
  res.write('   \
    ...
    The SYS_RATES price peg is updated anytime the SYS/BTC or USD/BTC price moves +/- 5% of the current values stored on the blockchain, this check is performed every ' + (config.updateInterval / 60) + ' minutes. \
    ...
    Values in the below are trimmed to 2 decimals. Full value can be seen in history here or on the blockchain. To support the Syscoin team price peg please send SYS to "SYS_RATES", all funds are used to cover alias update costs.</p>\
    \
